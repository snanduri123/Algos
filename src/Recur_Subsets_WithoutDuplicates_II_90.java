...
public class Recur_Subsets_WithoutDuplicates_II_90 {

    List<List<Integer>> answer;

    public List<List<Integer>> subsets(int[] nums) {
        Arrays.sort(nums); //so all the duplicates are together
    ...
    public void find_subsets(int[] nums, int pos, ArrayList<Integer> currSet) {
        ...
        for (int i = pos; i < nums.length; ) { //*** i< nums.length condition breaks the recursion.
            currSet.add(nums[i]);
            ...
            i++;
            while (i < nums.length && nums[i] == nums[i - 1]) { //skip if there is recently processed num again in input.
                i++;
            }
        }
    }
    ...
    public static void main(String[] args) {
        Recur_Subsets_WithoutDuplicates_II_90 n = new Recur_Subsets_WithoutDuplicates_II_90();
        System.out.println(Arrays.deepToString(n.subsets(new int[]{1, 2, 2}).toArray()));
    }
}
