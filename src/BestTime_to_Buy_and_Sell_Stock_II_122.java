...
public class BestTime_to_Buy_and_Sell_Stock_II_122 {
    ...
    public static int maxProfit(int[] prices) {
        ...
        for (int i = 0; i < prices.length; i++) {
            if (prices[i] < min) { //any curr price lesser than min price is min price
                min = prices[i];
            } else {
                profit += prices[i] - min; //every time you see higher price you sell(for profit) and keep it again as min price, anyways in future it will be overwritten if you see a lesser price.
                min = prices[i];
            }
        }
        return profit;
