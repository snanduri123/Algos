...
public class ConvertEdgeListToAdjacencyList {
    ...
    static ArrayList<ArrayList<Integer>> convert_edge_list_to_adjacency_list(Integer n, ArrayList<ArrayList<Integer>> edges) {
        ...
        for (int i = 0; i < edges.size(); i++) {
            ...
            if (edge.size() != 0) {
                ...
                ArrayList<Integer> adjacentList = adjacentLists.get(edge.get(0));
                adjacentList.add(edge.get(1));
                Collections.sort(adjacentList);

                //Below code is if input is undirected graph.
                //So first vertex is adjacent to second vertex, so update that in the firstVertex index.
                adjacentList = adjacentLists.get(edge.get(1));
                adjacentList.add(edge.get(0));
                Collections.sort(adjacentList);
            }
            //  }
        }
    ...
    public static void main(String[] args) {
        ConvertEdgeListToAdjacencyList c = new ConvertEdgeListToAdjacencyList();
