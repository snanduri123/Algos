...
return the earliest time slot that works for both of them and is of given duration.
...
public class MeetingScheduler_1229 {
    ...
    public List<Integer> minAvailableDuration(int[][] slots1, int[][] slots2, int duration) {
        ...
        for (int i = 0, j = 0; i < slots1.length && j < slots2.length; ) {
            if (slots1[i][1] > slots2[j][0]) { //if overlap
                ...
                if (overlapDuration >= duration) //check if there is enough duration
                    return Arrays.asList(overlapStart, overlapStart + duration);
            }
            if (slots1[i][1] < slots2[j][1]) {  //whoever ends first, that person's next interval has to be verified.
                i++;
            } else {
                j++;
            }
